...
class AnthropicLLMAdapter(BaseLLMAdapter[AnthropicLLMInvocationParams]):
    ...
    def _from_standard_message(self, message: LLMStandardMessage) -> MessageParam:
        ...
        becomes text blocks ahead of the ``tool_use`` blocks; blank text there
        adds none.
        ...
        if msg.get("tool_calls"):
            ...
            # message, as Anthropic returns it. Anthropic rejects a blank text
            ...
            ret = {
                "role": "assistant",
                "content": [{"type": "text", "text": text} for text in texts if text.strip()],
            }
            for tool_call in tc:
