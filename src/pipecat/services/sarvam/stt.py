...
class SarvamSTTService(STTService):
    ...
    async def _update_settings(self, delta: STTSettings) -> dict[str, Any]:
        ...
        if isinstance(delta, self.Settings) and not self._config.supports_vad_params:
            for param_name in (
                ...
                value = getattr(delta, param_name, NOT_GIVEN)
                if is_given(value) and value is not None:
                    raise ValueError(
                        f"Model '{self._settings.model}' does not support {param_name} parameter."
                    )
    ...
    async def _connect(self):
        ...
        try:
            ...
            connect_fn = self._sarvam_client.speech_to_text_streaming.connect
            if self._config.use_translate_endpoint:
                connect_fn = self._sarvam_client.speech_to_text_translate_streaming.connect

            try:
                self._websocket_context = connect_fn(
                    **connect_kwargs,
                    request_options=request_options,
                )
            except TypeError:
                # Fallback for SDK builds that don't expose request_options.
                self._websocket_context = connect_fn(**connect_kwargs)

            # Enter the async context manager
            self._socket_client = await self._websocket_context.__aenter__()

            if self._config.supports_prompt and self._settings.prompt is not None:
