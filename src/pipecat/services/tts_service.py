...
class TTSService(AIService):
        ...
        if pause_watchdog_timeout_s is not None:
            with warnings.catch_warnings():
                ...
                warnings.warn(
                    ...
                    "removed in 2.0.0. Frame processing now follows playback state.",
                    DeprecationWarning,
                    stacklevel=2,
                )
    ...
    async def _report_synthesis_timeout(self, context_id: str, message: str):
        ...
        self._failed_audio_contexts.add(context_id)
        logger.warning(f"{self}: {message}")
        await self.push_error(error_msg=message, force_treat_as_permanent=fatal)
    ...
    async def append_to_audio_context(
        ...
        if isinstance(frame, TTSAudioRawFrame):
            ...
            # report the same context again when it completes without audio.
            self._failed_audio_contexts.add(context_id)
    ...
    async def _record_context_audio_outcome(self, context_id: str, received_audio: bool):
        ...
        """
        if received_audio:
            self._failed_audio_contexts.discard(context_id)
