...
class TestPipelineWorkerLifecycle(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_end_with_parent_sends_bus_end_message(self):
        ...
        await parent.attach(registry=self.registry, bus=self.bus, worker_runner=self.runner)
...
class TestReachedFrameFilters(unittest.TestCase):
    def test_an_unset_filter_is_empty(self):
    ...
    def test_filter_deduplicates_in_caller_order(self):
        worker = make_stub_pipeline_task("filters")
    ...
    def test_setting_filter_replaces_previous_types(self):
        worker = make_stub_pipeline_task("filters")
