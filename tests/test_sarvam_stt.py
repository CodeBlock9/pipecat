...
class _LegacySocket:
    """Double matching the audio methods exposed by sarvamai 0.1.28."""
    ...
    async def transcribe(self, audio: str, encoding="audio/wav", sample_rate=16000):
        self.calls.append(("transcribe", audio, encoding, sample_rate))

    async def translate(self, audio: str, encoding="audio/wav", sample_rate=16000):
        self.calls.append(("translate", audio, encoding, sample_rate))

    async def set_prompt(self, prompt: str):
        self.calls.append(("set_prompt", prompt))
...
class _LegacyTranscriptionEndpoint:
    """Accept only the v2.5 transcription connection shape."""
...
class _LegacyTranslationEndpoint:
    """Accept only the v2.5 translation connection shape."""
...
def test_supported_models():
    """The upgraded service retains Dograh's deployed v2.5 configurations."""
...
@pytest.mark.parametrize("model", ["saarika:v2.5", "saaras:v2.5"])
def test_legacy_models_remain_constructible(model):
...
@pytest.mark.asyncio
async def test_saarika_v25_uses_transcription_endpoint_and_method(monkeypatch):
    socket = _LegacySocket()
    transcription = _LegacyTranscriptionEndpoint(socket)
    translation = _LegacyTranslationEndpoint(socket)
    service = SarvamSTTService(
        api_key="test-key",
    ...
    service._sarvam_client = SimpleNamespace(
        speech_to_text_streaming=transcription,
        speech_to_text_translate_streaming=translation,
    )
    ...
    monkeypatch.setattr(service, "create_task", discard_task)
    monkeypatch.setattr(service, "_create_keepalive_task", lambda: None)

    await service._connect()
    await _consume(service.run_stt(b"\x01\x02"))

    assert transcription.kwargs["model"] == "saarika:v2.5"
    assert transcription.kwargs["language_code"] == "hi-IN"
    assert "mode" not in transcription.kwargs
    assert "positive_speech_threshold" not in transcription.kwargs
    assert translation.kwargs is None
    assert socket.calls == [
        ("transcribe", base64.b64encode(b"\x01\x02").decode(), "audio/wav", 16000)
    ]


@pytest.mark.asyncio
async def test_saaras_v25_uses_translation_endpoint_and_post_connect_prompt(monkeypatch):
    socket = _LegacySocket()
    transcription = _LegacyTranscriptionEndpoint(socket)
    translation = _LegacyTranslationEndpoint(socket)
    service = SarvamSTTService(
    ...
    await service._connect()
    await _consume(service.run_stt(b"\x03\x04"))

    assert translation.kwargs["model"] == "saaras:v2.5"
    assert "language_code" not in translation.kwargs
    assert "mode" not in translation.kwargs
    assert "positive_speech_threshold" not in translation.kwargs
    assert transcription.kwargs is None
    assert socket.calls == [
        ("set_prompt", "prefer Dograh"),
        ("translate", base64.b64encode(b"\x03\x04").decode(), "audio/wav", 16000),
    ]
